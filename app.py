...
def popular_thread():
    threads = Thread.query.outerjoin(Answer).group_by(Thread.id).order_by(DB.db.func.count(Answer.id).desc()).limit(
        3).all()
    return jsonify([t.toJSON() for t in threads])
...
def add_answer():
    ...
    answer = Answer(content=request.args.get('content'), thread_id=request.args.get('thread_id'),
